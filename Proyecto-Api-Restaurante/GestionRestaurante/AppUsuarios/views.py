...
# Nueva vista para el inicio de sesión
class LoginView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if user is not None:
            login(request, user)  # Establecer la sesión de usuario (si usas SessionAuthentication)
            serializer = UserSerializer(user)
            if user.username == "admin":  # Verifica si el username es "admin" (minúscula)
                return Response(
                    {"message": "Inicio de sesión exitoso (Admin).", "usuario": serializer.data, "is_admin": True},
                    status=status.HTTP_200_OK
                )
            else:
                return Response(
                    {"message": "Inicio de sesión exitoso.", "usuario": serializer.data},
                    status=status.HTTP_200_OK
                )
        else:
            return Response(
